...
check:
	...
	$(MISC) clrty/dune-analytics-streamer.mis --check --compact-letters
	$(MISC) mis/kernel/DuneAnalyticsStreamerKernel.mis --check --compact-letters
	@find mis -name '*.mis' | while read f; do $(MISC) "$$f" --check --compact-letters || exit 1; done
	@echo OK dune-analytics-streamer
...
network-connect: commands-check
	...
	@echo MODULES mis/commands/DuneAnalyticsStreamerCommands.mis
